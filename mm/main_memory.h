...
#define TIMER_INTERVAL 50000                    /* microseconds */
...
typedef struct{
    ...
    int nr_free_frames;
    queue* disk_map;                            /* Map page table entries to secondary storage */
    bool thrashing;                             /* True means thrashing happening */
} main_memory;
...
/* Page fault and related functions */
...
/* Function that handles periodic interrups generated for maintainance of working set */
extern void working_set_interrupt_handler(int sig);

/* For block transfers to and from caches */
